...
parser.add_argument("--dataset", default="c100", type=str, help="[c10, c100]")
...
parser.add_argument("--mlp-hidden", default=2048, type=int)
...
if __name__ == "__main__":
    ...
    print("[INFO] Log with CSV and TensorBoard")
    ...
    # ModelCheckpoint 回调，每隔一定的 epoch 保存一次检查点
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"checkpoints/{experiment_name}",
        filename="{epoch:02d}-{val_loss:.2f}",
        save_top_k=-1,  # 保存所有检查点
        every_n_epochs=100  # 每隔多少个 epoch 保存一次
    )
    ...
    trainer = pl.Trainer(precision=args.precision,fast_dev_run=args.dry_run, gpus=args.gpus, benchmark=args.benchmark, logger=loggers, max_epochs=args.max_epochs, weights_summary="full", progress_bar_refresh_rate=refresh_rate, callbacks=[checkpoint_callback])
